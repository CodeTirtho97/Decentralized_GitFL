...
class SimpleCNN(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(3, 6, kernel_size=5, padding=2),  # Reduced from 16 to 6 filters
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(6, 12, kernel_size=5, padding=2),  # Reduced from 32 to 12 filters
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
        self.classifier = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(12 * 8 * 8, 64),  # Reduced from 120 to 64
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
